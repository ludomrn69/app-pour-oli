...
// Configuration SendGrid - utilise .env en local
...
async function sendEmailNotification(type, data) {
  ...
  let subject;
  if (type === "memories") {
    subject = "Un nouveau souvenir a été partagé sur Oli & Ludo";
  } else {
    subject = "Une nouvelle conversation a été ajoutée sur Oli & Ludo";
  }
  ...
  const msg = {
    ...
    replyTo: "ludovicmarion70@gmail.com",
    subject,
    text: "Bonjour,\n\n" +
    ...
    html: `
      ...
      margin: auto;">
        ...
        <p>Bonjour,</p>
        <p>
          <strong>${fromUser}</strong> a récemment partagé un nouveau
          ${typeLabel} sur votre espace personnel
          <strong>Oli & Ludo</strong>.
        </p>
        ...
        <p style="font-size: 0.9em; color: #888;">
          Ce message a été généré automatiquement par la plateforme
          Oli & Ludo.<br>
...
exports.notifyOnMemoryAdd = onDocumentCreated(
    ...
    async (event) => {
      try {
        ...
        const result =
          await sendEmailNotification("memories", data);
...
exports.notifyOnMessageAdd = onDocumentCreated(
    ...
    async (event) => {
      try {
        ...
        const result =
          await sendEmailNotification("conversations", data);
        console.log("Email sent successfully");
        return result;
      } catch (error) {
        console.error("Error sending email:", error);
